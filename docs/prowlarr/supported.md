---
title: Prowlarr Supported
description: List of supported indexers, trackers, and download clients compatible with Prowlarr
tags:
  - supported
  - indexers
  - trackers
  - download-clients
  - compatibility
  - prowlarr
---

!!! warning
    This page is a work in progress and requires additional effort.

This page is the disambiguation page for all `supported` wiki links (i.e. typically "more info" in the UI).

# Applications

{#applications}

Prowlarr can automatically sync indexers with various applications

- LazyLibrarian {#lazylibrarian}
  - [Refer to the Settings Page](settings.md#applications)
- Lidarr {#lidarr}
  - [Refer to the Settings Page](settings.md#applications)
- Mylar {#mylar}
  - [Refer to the Settings Page](settings.md#applications)
- Radarr {#radarr}
  - [Refer to the Settings Page](settings.md#applications)
- Readarr {#readarr}
  - [Refer to the Settings Page](settings.md#applications)
- Sonarr {#sonarr}
  - [Refer to the Settings Page](settings.md#applications)
- Whisparr {#whisparr}
  - [Refer to the Settings Page](settings.md#applications)

# Download Clients

{#downloadclient}

- Aria2 {#aria2}
  - [Refer to the Settings Page](settings.md#download-clients)
- Deluge {#deluge}
  - [Refer to the Settings Page](settings.md#download-clients)
- Download Station {#torrentdownloadstation}
  - [Refer to the Settings Page](settings.md#download-clients)
- Download Station {#usenetdownloadstation}
  - [Refer to the Settings Page](settings.md#download-clients)
- Flood {#flood}
  - [Refer to the Settings Page](settings.md#download-clients)
- Freebox Download {#torrentfreeboxdownload}
  - [Refer to the Settings Page](settings.md#download-clients)
- Hadouken {#hadouken}
  - [Refer to the Settings Page](settings.md#download-clients)
- NZBGet {#nzbget}
  - [Refer to the Settings Page](settings.md#download-clients)
- NZBVortex {#nzbvortex}
  - [Refer to the Settings Page](settings.md#download-clients)
- Pneumatic {#pneumatic}
  - [Refer to the Settings Page](settings.md#download-clients)
- qBittorrent {#qbittorrent}
  - [Refer to the Settings Page](settings.md#download-clients)
- rTorrent {#rtorrent}
  - [Refer to the Settings Page](settings.md#download-clients)
- SABnzbd {#sabnzbd}
  - [Refer to the Settings Page](settings.md#download-clients)
- Torrent Blackhole {#torrentblackhole}
  - [Refer to the Settings Page](settings.md#download-clients)
- Transmission {#transmission}
  - [Refer to the Settings Page](settings.md#download-clients)
- Usenet Blackhole {#usenetblackhole}
  - [Refer to the Settings Page](settings.md#download-clients)
- uTorrent {#utorrent}
  - [Refer to the Settings Page](settings.md#download-clients)
  - Due to utorrent being adware and formerly spyware, it is not reconmended. Most users use qBittorrent
- Vuze {#vuze}
  - [Refer to the Settings Page](settings.md#download-clients)

# Indexer Proxies

{#indexerproxy}

- FlareSolverr {#flaresolverr}
  - [Refer to the Settings Page](settings.md#flaresolverr-proxy-settings)
  - [See TRaSH's Guides on "How to setup Flaresolverr"](https://trash-guides.info/Prowlarr/prowlarr-setup-flaresolverr/)
- Http {#http}
  - [Refer to the Settings Page](settings.md#http-proxy-settings)
  - [See TRaSH's Guides on "Setup proxy for certain indexers"](https://trash-guides.info/Prowlarr/prowlarr-setup-proxy/)
- Socks4 {#socks4}
  - [Refer to the Settings Page](settings.md#socks4-proxy-settings)
  - [See TRaSH's Guides on "Setup proxy for certain indexers"](https://trash-guides.info/Prowlarr/prowlarr-setup-proxy/)
- Socks5 {#socks5}
  - [Refer to the Settings Page](settings.md#socks5-proxy-settings)
  - [See TRaSH's Guides on "Setup proxy for certain indexers"](https://trash-guides.info/Prowlarr/prowlarr-setup-proxy/)

# Notifications

{#notification}

- Apprise {#apprise}
- Custom Script {#customscript}
  - This allows you to make a custom script for when a particular action happens this script will run. See [Custom Scripts](custom-scripts.md) for more details.
- Discord {#discord}
  - By far one of the most common ways to push notifications of actions happening on your Prowlarr
- Email {#email}
  - Simply send yourself or somebody you want to annoy with email. If you're using Gmail, you need to enable less secure apps. If you're using Gmail and have 2-factor authentication enabled you need to use an App Specific password.
  !!! info
      - > You can use a "pretty address" like `SomePrettyName <email@example.org>`
- Gotify {#gotify}
- Join {#join}
- Mailgun {#mailgun}
- Notifiarr {#notifiarr}
- Ntfy {#ntfy}
- Prowl {#prowl}
- Pushbullet {#pushbullet}
- Pushcut {#pushcut}
- Pushover {#pushover}
- SendGrid {#sendgrid}
- Signal {#signal}
- Simplepush {#simplepush}
- Slack {#slack}
- Telegram {#telegram}
- Twitter {#twitter}
- Webhook {#webhook}
