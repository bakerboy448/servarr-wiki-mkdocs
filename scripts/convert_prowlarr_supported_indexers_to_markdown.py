        ...
        try:
            ...
            try:
                ...
                if country is not None:
                    ...
                    country = "Unknown"
                    _logger.warning(
                        "Unknown split country code [%s]: %s for %s",
                        language_code,
                        split_lang[1],
                        indexer,
                    )
            ...
            language = "Unknown"
            _logger.warning(
                "Unknown split language code [%s]: %s for %s",
                language_code,
                split_lang[0],
                indexer,
            )
...
def build_markdown_table(indexers, privacy, protocol):
    ...
    for indexer in indexers:
        if indexer["privacy"] in privacy and indexer["protocol"] == protocol:
            ...
            if (
                indexer["indexerUrls"][0] is not None
                and indexer["indexerUrls"][0] != ""
            ):
...
def extract_and_compare_commits_from_file(filename, app_commit, indexer_commit):
    ...
    try:
        with open(filename, "r") as content_file:
            ...
            prev_indexer_commit_match = re.search(indexer_commit_pattern, content)

            prev_app_commit = (
                prev_app_commit_match.group(1) if prev_app_commit_match else None
            )
            prev_indexer_commit = (
                prev_indexer_commit_match.group(1)
                if prev_indexer_commit_match
                else None
            )
            ...
            if prev_indexer_commit:
                _logger.info("Existing Indexer Commit is {%s}", prev_indexer_commit)
    except:
...
def main(
    ...
    compared_tables = hash_and_compare_tables(hashfile, tbl_fmt_use, tbl_fmt_tor)
    if compared_tables:
    ...
    header_wiki = (
        ...
        "description: Indexers currently named as supported in the current nightly build of Prowlarr. "
        "Other indexers may be available via either Generic Newznab or Generic Torznab.\n"
        "tags:\n"
        "  - prowlarr\n"
        "  - indexers\n"
        "  - reference\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        "--hashfile",
        help="Hash file of previous wiki page tables",
    ...
    parser.add_argument("-pc", "--prev_app_commit", help="Previous App Commit hash")
    parser.add_argument(
        "-pic", "--prev_indexer_commit", help="Previous Indexer Commit hash"
    )
